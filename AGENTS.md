...
Atlas Method is a lean-by-design documentation methodology and template for running a personal operating system on Claude Code; done means the public repo (README, versions/, templates, hooks, docs) stays internally consistent and installable, and scope stops at the public template - it does not include any of Damian's private domain instances built from it.
...
Zero-dependency Node.js scripts (structural gate, install scripts)
Bash hooks (agent-prelude injection, pre-read, wrap-push reminders)
No build step, no package manager dependencies for the shipped protocol itself

## Hard rules

No em dashes or en dashes in any tracked file - use " - " instead
VERSION file at repo root is the single source of truth for the public release version - every other surface (README, etc.) must match it exactly
Media files (audio/video/large binaries) are never committed to this repo
master is protected - all changes land via branch + PR, no direct pushes to master
...
```bash
node scripts/check-atlas-method.js
```
